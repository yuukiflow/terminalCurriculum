...
function tabComplete(command) {
    try {
        ...
        if (searchPath.includes('/')) {
            ...
            if (!searchNode) {
                writeToTerminal('', `<span class="error">No such directory: ${parentPath}</span>`);
                return;
            }
            ...
            if (matches.length === 0) {
                writeToTerminal('', `<span class="error">No matches found for: ${searchTerm}</span>`);
                return;
            }
            ...
            if (matches.length === 1) {
                ...
                const matchList = matches.map(match => {
                    const isFolder = searchNode.children[match].type === 'folder';
                    return `<span class="${isFolder ? 'folder' : ''}">${parentPath}/${match}${isFolder ? '/' : ''}</span>`;
                }).join('<br>');
                writeToTerminal('', `<div class="matches">Possible matches:<br>${matchList}</div>`);
            }
            return;
        }
        ...
        if (matches.length === 0) {
            writeToTerminal('', `<span class="error">No matches found for: ${searchPath}</span>`);
            return;
        }
        ...
        if (matches.length === 1) {
            ...
            const matchList = matches.map(match => {
                const isFolder = searchNode.children[match].type === 'folder';
                return `<span class="${isFolder ? 'folder' : ''}">${match}${isFolder ? '/' : ''}</span>`;
            }).join('<br>');
            writeToTerminal('', `<div class="matches">Possible matches:<br>${matchList}</div>`);
        }
    } catch (error) {
        console.error('Tab completion error:', error);
        writeToTerminal('', `<span class="error">Error during tab completion: ${error.message}</span>`);
    }
}
